...
data11.columns  # ['Country', 'Happiness_Rank', 'Happiness_Score', 'year']

# (1) 3년 연속 행복지수가 기록된 국가
q1_agg = data11.groupby('Country').apply(len)
# apply() ->  행 또는 열에 원하는 연산을 지원. 열 단위는 axis=0
q1_agg[q1_agg < 3]  # 3미만인 국가 출력
len(q1_agg[q1_agg < 3])
...
# (1) 연도별 행복지수
q1_tab = pd.pivot_table(data=data11, index='Country',
                        columns='year', values='Happiness_Score')

q2 = q1_tab.dropna()  # 3년 연속 데이터가 기록되지 않은 국가 데이터는 제외

# (2) 행복지수 증감률을 산출
q2.loc[:, 'ratio'] = (q2.loc[:, 2017] - q2.loc[:,2015])/2
# loc() -> 행을 조회, loc[:, 'ratio'] -> 모든 row의 ratio를 인덱싱


# (3) 행복지수 증감률이 가장 높은 3개 국가
...
# 서로 다른 개수의 데이터를 가진 3개 집단에 대해 평균 차이가 있는지 일원분산분석
...
# ols(종속변수 ~ 독립변수, data) -> 최소제곱법

anova_lm(ols1)  # ols를 인수로 넣어야함
...
x_var = ['Age', 'Dependent_Count', 'is_Married', 'Read_Book_per_Year', 'Income_Range']
q1 = data12[x_var].corr()
q1.drop('Read_Book_per_Year')['Read_Book_per_Year'].abs().max()
...
q2 = data12.copy()
q2_1 = q2[(q2['Education_Level']=='석사')]['Read_Book_per_Year']
q2_2 = q2[(q2['Education_Level']=='박사')]['Read_Book_per_Year']

from scipy.stats import bartlett
bartlett(q2_1, q2_2)
# pvalue=0.028949506571599563

from scipy.stats import ttest_ind
q2_out = ttest_ind(q2_1, q2_2, equal_var=False)
q2_out.statistic
